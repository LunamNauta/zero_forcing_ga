...
void test_c4() {
    ...
    VertexSet initial = {0};
    assert(graph.zf_closure(initial).size() == 1);
    // Two adjacent vertices force the whole cycle
    initial = {0, 1};
    assert(graph.zf_closure(initial).size() == 4);
...
void test_star5() {
    ...
    initial = {0, 1, 2, 3};
    assert(graph.zf_closure(initial).size() == 5);
...
void test_complete_k4() {
    ...
    assert(graph.zf_closure(initial).size() == 2);
    initial = {0, 1, 2};
    assert(graph.zf_closure(initial).size() == 4);
